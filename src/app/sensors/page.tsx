...
import {SensorHistoryProvider, useSensor24hMinMax} from '@/components/sensors/SensorHistoryProvider';
...
const CATEGORY_ORDER = ['Battery & Charging', 'Temperatures', 'Mow Motor', 'GPS', 'Other'] as const;
...
// voltage/temperature drift meaningfully over a day; current and GPS accuracy are just
// noisy (current is mostly 0 when idle, GPS accuracy jumps around) — min/max there isn't useful
function hasMeaningful24hRange(info: SensorInfo): boolean {
  return info.value_description === 'VOLTAGE' || info.value_description === 'TEMPERATURE';
}
...
const SensorGauge = memo(function SensorGauge({info, raw}: {info: SensorInfo; raw: string | undefined}) {
  ...
  // Voltage gauges turned out to add no real value (see PR discussion) — the useful ranges are
  // tiny slivers of whatever domain we'd pick, so it's just a plain number for those now.
...
const SensorCard = memo(function SensorCard({info}: {info: SensorInfo}) {
  ...
  const showMinMax = hasMeaningful24hRange(info);
  const minMax24h = useSensor24hMinMax(info.sensor_id);
  ...
  return (
    <Card sx={{...outerCardStyles(theme), minWidth: 160, flex: '1 0 160px'}}>
      <CardContent sx={{display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 1}}>
        ...
        {showMinMax && minMax24h && (
          <Tooltip title="Min/Max der letzten 24 Stunden">
            <Typography variant="caption" color="text.secondary" sx={{cursor: 'default'}}>
              {minMax24h.min.toFixed(2)}–{minMax24h.max.toFixed(2)} {formatUnit(info.unit)}
            </Typography>
          </Tooltip>
        )}
      </CardContent>
...
// signature of currently-critical sensor ids in this category, as a string so the
// default equality check only re-renders when the critical set actually changes
function useCriticalIds(sensors: SensorInfo[]): Set<string> {
  const signature =
    useSelectedMower((m) => {
      if (!m) return '';
      return sensors
        .filter((info) => isCritical(info, m.sensorData[info.sensor_id], m.state.current_state))
        .map((info) => info.sensor_id)
        .join(',');
    }) ?? '';
...
function CategorySection({category, sensors}: {category: string; sensors: SensorInfo[]}) {
  const isBatteryCategory = category === 'Battery & Charging';
  const currentState = useSelectedMower((m) => m?.state.current_state);
  const motorOff = category === 'Mow Motor' && currentState !== 'MOWING';
  const baseVisible = isBatteryCategory || motorOff ? [] : sensors.filter((info) => !BATTERY_CATEGORY_SENSOR_IDS.has(info.sensor_id));
  const visible = useMemo(() => {
    if (category !== 'Temperatures') return baseVisible;
  ...
  }, [baseVisible, category]);
  const criticalIds = useCriticalIds(visible);

  const sorted = useMemo(
    () => [...visible].sort((a, b) => Number(criticalIds.has(b.sensor_id)) - Number(criticalIds.has(a.sensor_id))),
    [visible, criticalIds],
  );
...
export default function SensorsPage() {
  ...
  return (
    <SensorHistoryProvider mowerId={mowerId}>
      <Page>
